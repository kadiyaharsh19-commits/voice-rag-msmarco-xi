    ...
    def _resolve_host(self) -> str:
        ...
        try:
            with urllib.request.urlopen(req, timeout=self.cfg.timeout_s) as resp:
                ...
                if host:
                    return f"https://{host}"
                raise RuntimeError(f"Could not find host for Pinecone index '{self.cfg.index_name}'")
        except Exception as e:
            # Fallback format for standard Pinecone serverless hosts
            return f"https://{self.cfg.index_name}.svc.pinecone.io"
